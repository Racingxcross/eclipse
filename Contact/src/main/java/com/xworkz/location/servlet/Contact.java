...
@WebServlet(loadOnStartup = 2, urlPatterns = "/talk")
public class Contact extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String comments = req.getParameter("gossip");
		...
		if (email.length() > 5) {
			...
			write.print("<span style='color:red'/>");
			write.print("Email is not valid");
		}
		...
		if (convLong > 888888) {
			...
			write.print("<span style='color:red'/>");
			write.print("Mobile number is not valid");
		}
		...
		if (comments.length() > 15) {
			...
			write.print("<span style='color:red'/>");
			write.print("Comments is not valid");
		}
